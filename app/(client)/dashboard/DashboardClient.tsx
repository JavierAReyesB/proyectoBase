    ...
    {AVAILABLE_WIDGETS.map(({ type, icon: Icon, title, description }) => {
      ...
      return (
        <Button
          ...
          className={`w-full h-auto justify-start p-4 ${selected ? "ring-2 ring-primary" : ""}`}
          onClick={() => {
            if (selected) {
              dash.setSelectedWidgets(dash.selectedWidgets.filter((t) => t !== type));
            } else {
            ...
            >
              {dash.widgets.map((w) => (
                ...
                >
                  ...
                  <ResizableCard className="h-full w-full">
                    <ResizableCardHeader
                      className="flex flex-row items-center justify-between pb-2 cursor-grab active:cursor-grabbing"
